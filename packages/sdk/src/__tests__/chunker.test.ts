...
describe('defaultChunker', () => {
  it('returns single chunk for short content', async () => {
    const document = createTestDocument({ content: 'Short text.' })
    const chunks = await defaultChunker(document, { chunkSize: 100, chunkOverlap: 20 })
  ...
  it('splits long content into multiple chunks', async () => {
    ...
    const chunks = await defaultChunker(document, { chunkSize: 512, chunkOverlap: 0 })
  ...
  it('does not break mid-sentence', async () => {
    ...
    const chunks = await defaultChunker(document, { chunkSize: 64, chunkOverlap: 0 })
